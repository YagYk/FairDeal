        ...
        {scripts.map((script, index) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <h4 className="font-semibold text-gold-500 mb-1">{script.clause}</h4>
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs border ${getProbabilityBg(script.successProbability)}`}>
                  <span className={getProbabilityColor(script.successProbability)}>
                    {Math.round(script.successProbability * 100)}% Success Probability
                  </span>
                </div>
              </div>
              <motion.button
